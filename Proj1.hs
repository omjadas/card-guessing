...
feedback targets guesses = (feedback1 targets guesses,
...
feedback3 targets guesses
    | otherwise = sum [min (length t_rank) (length g_rank) |
                       t_rank <- t_ranks,
                       g_rank <- g_ranks,
                       t_rank !! 0 == g_rank !! 0]
  where
...
feedback5 targets guesses = sum [min (length t_suit) (length g_suit) |
                                 t_suit <- t_suits,
                                 g_suit <- g_suits,
                                 t_suit !! 0 == g_suit !! 0]
  where
...
initialGuess n = (guess, (GameState all_guesses))
  where 
    all_guesses = subseqOfSize n ([minBound..maxBound]::[Card])
...
nextGuess (guess, guesses) f = (next_guess, new_state)
  where
    new_state = GameState (delete guess (not_guessed guesses))
    possible_guesses = [g |
                        g <- not_guessed new_state,
                        (feedback g guess) == f]
